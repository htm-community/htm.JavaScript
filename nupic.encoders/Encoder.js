...
var Encoder = function() {
    ...
    this.clipInput = false; // boolean
    /** if true, skip some safety checks (for compatibility reasons), default false */
    this.forced = false; // boolean
    /** Encoder name - an optional string which will become part of the description */
    this.name = ""; // String
    this.padding = 0; // int
    this.nInternal = 0; // int
    this.rangeInternal = 0; // double
    this.range = 0; // double
    this.encLearningEnabled = false; // boolean
    this.flattenedFieldTypeList = null; // List<FieldMetaType>
    this.decoderFieldTypes = null; // Map<Tuple, List<FieldMetaType>>
    /**
     * This matrix is used for the topDownCompute. We build it the first time
     * topDownCompute is called
     */
    this.topDownMapping = null; // SparseObjectMatrix<int[]>
    this.topDownValues = null; // double[]
    this.bucketValues = null; // List<?>
    this.encoders = null; // LinkedHashMap<EncoderTuple, List<EncoderTuple>>
    this.scalarNames = null; // List<String>
};
...
Encoder.prototype = {
    ...
    getEncoderTuple: function(e) { // EncoderTuple(Encoder<T>)
        ...
        var tuples = Array.from(this.encoders.keys());
        for (var i = 0; i < tuples.length; i++) {
            if (tuples[i].getEncoder().equals(e)) {
                return tuples[i];
            }
        }
        return null;
    ...
    getScalarNames: function(parentFieldName) { // List<String>(String)
        ...
        if (!isNullOrUndefined(this.getEncoders())) {
            ...
            for (var i = 0; i < hierarchicalNames.length; i++) {
                var name = hierarchicalNames[i];
                names.push(name);
            }
        } else {
            if (!isNullOrUndefined(parentFieldName)) {
    ...
    getEncoderList: function() { // List<Encoder<T>>(void)
        ...
        if (!isNullOrUndefined(registeredList) && registeredList.length !== 0) {
            for (var i = 0; i < registeredList.length; i++) {
                var t = registeredList[i];
                var subEncoders = t.get(1).getEncoderList();
                for (var j = 0; j < subEncoders.length; j++) {
                    var subEncoder = subEncoders[j];
    ...
    getEncodedValues: function(inputData) { // <S> List<String>(S)
        ...
        if (!isNullOrUndefined(encoders) && encoders.length > 0) {
            var ts = Array.from(encoders.keys());
            for (var i = 0; i < ts.length; i++) {
                var vs = ts[i].getEncoder().getEncodedValues(inputData);
    ...
    getBucketIndices: function(input) { // int[](String) or int[](double)
        ...
        if (!isNullOrUndefined(encoders) && encoders.length > 0) {
            var ts = Array.from(encoders.keys());
            for (var i = 0; i < ts.length; i++) {
                var vs = ts[i].getEncoder().getBucketIndices(input);
                for (var j = 0; j < vs.length; j++) {
                    var v = vs[j];
                    l.push(v);
                }
    ...
    scalarsToStr: function(scalarValues, scalarNames) { // String(List<?>, List<String>)
        ...
        desc = "";
        var ts = ArrayUtils.zip(scalarNames, scalarValues);
        for (var i = 0; i < ts.length; i++) {
            var t = ts[i];
            if (desc.length > 0) {
    ...
    pprintHeader: function(prefix) { // void(String)
        ...
        for (var i = 0; i < len; i++) {
            ...
            var width = Math.floor(description[i + 1].get(1));
    ...
    pprint: function(output, prefix) { // void(int[], String)
        ...
        var len = description.length - 1;
        for (var i = 0; i < len; i++) {
            var offset = Math.floor(description[i].get(1));
            var nextOffset = Math.floor(description[i + 1].get(1));
    ...
    decode: function(encoded, parentFieldName) { // Tuple(int[], String)
        ...
        for (var i = 0; i < len; i++) {
            ...
            var fieldOutput = ArrayUtils.sub(encoded, ArrayUtils.range(Math.floor(threeFieldsTuple.get(2)), nextOffset));

            var result = threeFieldsTuple.get(1).decode(fieldOutput, parentName);

            for (var key of result.get(0).keys()) {
                fieldsMap.set(key, result.get(0).get(key));
            }
            var vs = result.get(1);
            for (var j = 0; j < vs.length; j++) {
                fieldsOrder.push(vs[j]);
            }
        }
    ...
    getBucketInfo: function(buckets) { // List<EncoderResult>(int[])
        ...
        var retVals = [];
        var bucketOffset = 0;
        var encoderTuples = this.getEncoders(this);
        for (var i = 0; i < encoderTuples.length; i++) {
            var encoderTuple = encoderTuples[i];
    ...
    closenessScores: function(expValues, actValues, fractional) { // TDoubleList(TDoubleList, TDoubleList, boolean)
        ...
        var scalarIdx = 0;
        var ress = this.getEncoders(this);
...
Encoder.prototype.Builder.w = function(w) {
    this.w = w;
    return this;
};

Encoder.prototype.Builder.minVal = function(minVal) {
    this.minVal = minVal;
    return this;
};

Encoder.prototype.Builder.maxVal = function(maxVal) {
    this.maxVal = maxVal;
    return this;
};

Encoder.prototype.Builder.radius = function(radius) {
    this.radius = radius;
    return this;
};

Encoder.prototype.Builder.resolution = function(resolution) {
    this.resolution = resolution;
    return this;
};

Encoder.prototype.Builder.periodic = function(periodic) {
    this.periodic = periodic;
    return this;
};

Encoder.prototype.Builder.clipInput = function(clipInput) {
    this.clipInput = clipInput;
    return this;
};

Encoder.prototype.Builder.forced = function(forced) {
    this.forced = forced;
    return this;
};

Encoder.prototype.Builder.name = function(name) {
    this.name = name;
    return this;
};